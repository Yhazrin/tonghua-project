...
export default function AfterSalesPage() {
  ...
  const columns: Column<AfterSales>[] = [
    ...
    { key: 'order_id', title: '订单 ID', width: 120 },
    { key: 'user_id', title: '用户 ID', width: 120 },
    { key: 'type', title: '类型', width: 100, render: (v) => typeLabels[v] || v },
    { key: 'reason', title: '原因', width: 200, render: (v) => (v ? String(v).slice(0, 40) + (String(v).length > 40 ? '…' : '') : '-') },
    { key: 'status', title: '状态', width: 120, render: (v) => <StatusBadge status={v} /> },
    { key: 'created_at', title: '申请时间', width: 180, render: (v) => dayjs(v).format('YYYY-MM-DD HH:mm') },
  ];

  return (
    <div>
      <div style={{ marginBottom: 20 }}>
        <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 4, fontFamily: 'var(--font-serif)' }}>售后管理</h1>
